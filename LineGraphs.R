...
# The line below loads from a file, but you can also
# use the drop-down as I'll demonstrate.
#####################################################
...
#####################################################
# Now, let's compare three years of the data by pulling 
...
library(lubridate)  # contains a function to extract a month from a date
                    ...
                    # us a readable graph
ggplot(cIndexThreeYears, month(Date), value, color=factor(Year))) + 
  geom_line() + theme_bw() + 
  scale_x_continuous(limits=c(1,12), breaks=1:12)

# as an alternative to using the lubridate library
ggplot(cIndexThreeYears, aes( as.integer(Month), value, color=factor(Year))) + 
  geom_line() + theme_bw() + 
  scale_x_continuous(limits=c(1,12), breaks=1:12)
